...
class Controller extends System
{
    ...
    private function setPath($render)
    {
        ...
        if (is_array($render)) {
            foreach ($render as $li) {
                $path = 'public/view/' . $this->getArea() . '/' . $this->getController() . '/' . $li . '.phtml';
                $this->fileExists($path);
                $this->path[] = $path;
            }
        } else {
            $this->pathRender = is_null($render) ? $this->getAction() : $render;
            $this->path = 'public/view/' . $this->getArea() . '/' . $this->getController() . '/' . $this->pathRender . '.phtml';
    ...
    public function view($render = null)
    {
        ...
        if (is_null($this->layout)) {
            ...
            $this->layout = 'resources/' . $this->getArea() . '/layouts/' . $this->layout . '.phtml';
            if (file_exists($this->layout)) {
                $this->render($this->layout);
            } else {
                header('HTTP/1.0 404 Not Found');
                define('APP_ERROR_CODE', '404');
                define('APP_ERROR', 'Não foi possível localizar o layout desejado');
                include("./resources/" . $this->getArea() . "/layouts/error.phtml");
                die();
            }
        }
